...
let pattern = new Tone.Pattern((time, note)=>{
  if (random(10) > 6 ) {
    synth2.triggerAttackRelease(patternNotes[Math.floor(random(5))], "4n", time);
  }
  synth2.triggerAttackRelease(note, "8n", time+0.01);
}, ["C5", "D5", "E5", "G5", "A5"]);
...
function setup() {
  ...
  midi = new Midi.fromUrl("TheLegend_Of_Zelda-Title.mid")
